...
def video_reproject(orig_dir_path, recon_dir_path, save_dir_path, state_dir_path, seamless=False):
    ...
    for idx in range(counter):
        ...
        crop_size_ = (quad_0[3] - quad_0[1], quad_0[2] - quad_0[0])
        ...
        print (crop_size, coeffs,)

        x = transforms.ToTensor()(img_2)
        # x = transforms.Resize(crop_size_)(x)

        x = torchvision.transforms.functional_tensor.perspective(x, torch.from_numpy(coeffs))
        print (x.shape)
        x = x[:,:crop_size_[0],:crop_size_[1]]        
        
        y = transforms.ToTensor()(img_0)
        y[:,int(quad_0[1]):int(quad_0[3]),int(quad_0[0]) : int(quad_0[2])] = x



        img_2 = img_2.transform(crop_size, Image.PERSPECTIVE, coeffs, Image.BICUBIC)
        #print (np.array(pil_to_cv2(img_2)).sum())
        ...
        if not seamless:
            ...
            utils.save_image(
                x-transforms.ToTensor()(img_2),
                save_dir_path + img_list_2[idx].replace(".",'_.'),
                normalize=False,
                range=(-1, 1),
            )
